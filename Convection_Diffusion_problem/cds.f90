!Code for Steady Convection-Diffusion equation using CDS scheme for convective term

module functions

implicit none

	integer, parameter:: nx = 20, ny = 20				!Set grid size here; ny-->refers horizontal direction, nx-->refers vertical direction
	integer:: i, j, iter, t
...
contains
	...
	implicit none
		...
		do i = 2, nx-1
			do j = 2, ny-1
				...
				aE(i, j) = De(i, j) - (Fe(i, j) * 5.0d-1)
				aW(i, j) = Dw(i, j) + (Fw(i, j) * 5.0d-1)
				aN(i, j) = Dn(i, j) - (Fn(i, j) * 5.0d-1)
				aS(i, j) = Ds(i, j) + (Fs(i, j) * 5.0d-1)
				
				!Calculating b term
				b(i, j) = 2.0d0 * dx * dy * (x_c(j) + y_c(i))
	...
	implicit none
		...
		do j = 2, ny-1
			...
			!Calculating neighbouring coefficients
			aE(nx, j) = De(nx, j) - (Fe(nx, j) * 5.0d-1)
			aW(nx, j) = Dw(nx, j) + (Fw(nx, j) * 5.0d-1)
			aN(nx, j) = Dn(nx, j) - (Fn(nx, j) * 5.0d-1)
			aS(nx, j) = 0.0d0
	...
	implicit none
		...
		do i = 2, nx-1
			...
			!Calculating neighbouring coefficients
			aE(i, 1) = De(i, 1) - (Fe(i, 1) * 5.0d-1)
			aW(i, 1) = 0.0d0
			aN(i, 1) = Dn(i, 1) - (Fn(i, 1) * 5.0d-1)
			aS(i, 1) = Ds(i, 1) + (Fs(i, 1) * 5.0d-1)
	...
	implicit none
		...
		do j = 2, ny-1
			...
			!Calculating neighbouring coefficients
			aE(1, j) = De(1, j) - (Fe(1, j) * 5.0d-1)
			aW(1, j) = Dw(1, j) + (Fw(1, j) * 5.0d-1)
			aN(1, j) = 0.0d0
			aS(1, j) = Ds(1, j) + (Fs(1, j) * 5.0d-1)
			
			!Calculating b term
			b(1, j) = (2.0d0 * dx * dy * (x_c(j) + y_c(1))) 
	...
	implicit none
		...
		do i = 2, nx-1
			...
			!Calculating neighbouring coefficients
			aE(i, ny) = 0.0d0
			aW(i, ny) = Dw(i, ny) + (Fw(i, ny) * 5.0d-1)
			aN(i, ny) = Dn(i, ny) - (Fn(i, ny) * 5.0d-1)
			aS(i, ny) = Ds(i, ny) + (Fs(i, ny) * 5.0d-1)
			
			!Calculating b term
			b(i, ny) = (2.0d0 * dx * dy * (x_c(ny) + y_c(i))) 
	...
	implicit none
		...
		!Calculating neighbouring coefficients
		aE(nx, 1) = De(nx, 1) - (Fe(nx, 1) * 5.0d-1)
		aW(nx, 1) = 0.0d0
		aN(nx, 1) = Dn(nx, 1) - (Fn(nx, 1) * 5.0d-1)
		aS(nx, 1) = 0.0d0
	...
	implicit none
		...
		!Calculating neighbouring coefficients
		aE(nx, ny) = 0.0d0
		aW(nx, ny) = Dw(nx, ny) + (Fw(nx, ny) * 5.0d-1)
		aN(nx, ny) = Dn(nx, ny) - (Fn(nx, ny) * 5.0d-1)
		aS(nx, ny) = 0.0d0
	...
	implicit none
		...
		!Calculating neighbouring coefficients
		aE(1, 1) = De(1, 1) - (Fe(1, 1) * 5.0d-1)
		aW(1, 1) = 0.0d0
		aN(1, 1) = 0.0d0
		aS(1, 1) = Ds(1, 1) + (Fs(1, 1) * 5.0d-1)
	...
	implicit none
		...
		!Calculating neighbouring coefficients
		aE(1, ny) = 0.0d0
		aW(1, ny) = Dw(1, ny) + (Fw(1, ny) * 5.0d-1)
		aN(1, ny) = 0.0d0
		aS(1, ny) = Ds(1, ny) + (Fs(1, ny) * 5.0d-1)
			
		!Calculating b term
		b(1, ny) = (2.0d0 * dx * dy * (x_c(ny) + y_c(1)))  
	...
	implicit none
		...
		fname1 = 'cds_converged_solution_'//trim(nxx)//'_'//trim(nyy)//'.txt'
		fname2 = 'cds_horizontal_center_line_data_'//trim(nxx)//'_'//trim(nyy)//'.txt'
		fname3 = 'cds_solver_runtime_details_'//trim(nxx)//'_'//trim(nyy)//'.txt'

!Code for Finite Volume LBM general poisuelle-couette flow using structured Cartesian grid using D2Q9 lattice type
...
implicit none
	...
	integer, parameter:: n = 100, m = 100, q = 9				!Domain with Nx = n, Ny = m cells
	double precision:: L = 1.0d0, H = 5.0d-1				!L = length of domain, H = height of domain		
	double precision, dimension(:, :, :), allocatable:: f_node, feq_node, RHS, S	
	...
	double precision, dimension(:), allocatable:: x_f, y_f, x_c, y_c, uexact, unum, y_vertical
	double precision:: t1, t2, t3, t4, num, denom
	double precision:: tau, nu, dx, dy, dt, tolerance, l2norm, Uplate, Re, dpdx, P, rhomax
...
contains
	...
	implicit none
		...
		allocate (f_node(1:q, 1:n, 1:m), feq_node(1:q, 1:n, 1:m), RHS(1:q, 1:n, 1:m), S(1:q, 1:n, 1:m))
		...
		allocate (x_f(0:n), y_f(0:m), x_c(1:n), y_c(1:m), uexact(0:m+1), unum(0:m+1), y_vertical(0:m+1))
	...
	implicit none
		...
		deallocate (f_node, feq_node, RHS, S)
		...
		deallocate (x_f, y_f, x_c, y_c, uexact, unum, y_vertical)
		deallocate (w, ex, ey)
	...
	implicit none
		...
		dx = L / n
		dy = H / m
		dt = 1.0d-4					!Set time step size
		steps = 1
		tolerance = 1.0d-7				!Set tolerance value 
		Uplate = 5.0d-2					!Set plate velocity
		dpdx = 1.0d-5					!Set pressure gradient
		Re = 15
		nu = Uplate * H / Re
		tau = 3.0d0 * nu
		P = (-dpdx * H**2) / (2.0d0 * Uplate * nu) 
		
		print*, 'Re = ', Re
		print*, 'viscosity = ', nu
		print*, 'Non-dimensional parameter P = ', P
	...
	implicit none
		...
		!Right wall -- periodic condition, Left wall -- periodic condition
		do j = 1, m
			do k = 1, q
				f_gc_face_right(k, j) = 0.50d0 * (f_node(k, n, j) + f_node(k, 1, j))
				f_gc_face_left(k, j) = 0.50d0 * (f_node(k, 1, j) + f_node(k, n, j))
			end do
		end do
		...
		do i = 1, n
			do j = 1, m	
				do k = 1, q
					...
					S(k, i, j) = (1.0d0 - 5.0d-1 / tau) * w(k) * dpdx * (t3 + t4)
				end do
			end do
		end do
	...
	implicit none
		...
		do i = 1, n
			do j = 1, m
				do k = 1, q
					RHS(k, i, j) = S(k, i, j) - ((1.0d0 / tau) * (f_node(k, i, j) - feq_node(k, i, j))) - &
	...
	implicit none
		...
		do i = 1, n
			do j = 1, m
				...
				u(i, j) = (t1 / rho(i, j)) + (dpdx * 5.0d-1 * dt / rho(i, j))
				v(i, j) = t2 / rho(i, j)
			end do
		end do
	...
	implicit none
		...
		do while (l2norm .gt. tolerance .or. steps .lt. 100)	
			...
			if (mod(steps, 1000) .eq. 0) then
				write(*, 1) steps * dt, l2norm, u(n/2, 10), u(n/2, 20), u(n/2, 40), u(n/2, m)
				1 format(6f15.10)
			endif
		end do			
	...
	implicit none
		...
		nx = trim(adjustl(nx))
		ny = trim(adjustl(ny))
		fname1 = 'uv_field_gpc_c4_'//trim(nx)//'_'//trim(ny)//'.txt'
		fname2 = 'u_prof_gpc_c4_'//trim(nx)//'_'//trim(ny)//'.txt'
		...
		uexact(m+1) = Uplate
		do j = 1, m
			uexact(j) = ((P * y_vertical(j) * (1.0d0 - y_vertical(j))) + y_vertical(j)) * Uplate
		end do
		
		!Extracting numerical data at n = 50
		unum(0) = 0.0d0
		unum(m+1) = Uplate
		do j = 1, m
			unum(j) = u(50, j)
		...
		do j = 0, m+1
			write(12, *) y_vertical(j), unum(j) / Uplate
		end do
